...
def     RISCVCore(imem_data, dmem_data, rf, reset, clock, env):
    ...
    u_InstMem = Rom(sig.instruction, sig.PC, imem_data)
    u_Adder1 = Adder(sig.PC4, sig.PC, sig.Const4)
    u_ImmGen = ImmGen(sig.immediate, sig.instruction)
    u_Control = MainControl(sig.opcode, sig.ALUOp, sig.ALUSrc, sig.Branch, sig.MemRead, sig.MemWrite, sig.MemtoReg, sig.RegWrite)
    ...
    u_DataMem = Ram(sig.MemReadData, sig.ReadData2, sig.ALUResult, sig.MemRead, sig.MemWrite, dmem_data, clock)
